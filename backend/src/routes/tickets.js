...
// Complete a ticket (for IN_REVIEW status)
router.post('/:id/complete', async (req, res) => {
  try {
    const ticketId = req.params.id;
    const { adminUserId } = req.body;

    // Get the ticket first to check status
    const ticket = await prisma.ticket.findUnique({
      where: { id: ticketId },
      select: { status: true, technicianNote: true },
    });
    ...
    if (ticket.status !== 'IN_REVIEW') {
      return res.status(400).json({ 
        error: 'Only tickets with IN_REVIEW status can be completed this way' 
      });
    }

    // Get admin name for approval note
    let approvalNote = '';
    if (adminUserId) {
      const admin = await prisma.adminUser.findUnique({
        where: { id: parseInt(adminUserId) },
        select: { name: true, email: true },
      });
      const adminName = admin?.name || admin?.email || 'Admin';
      const currentDate = new Date().toISOString().split('T')[0]; // Format: YYYY-MM-DD
    ...
    // Update ticket to COMPLETED status (admin completion)
    const updatedTicket = await prisma.ticket.update({
      where: { id: ticketId },
      data: {
        status: 'COMPLETED',
        completionTime: new Date(), // Admin completion time
        technicianNote: (ticket.technicianNote || '') + approvalNote,
      },
      include: {
        team: {
          include: { leader: true },
    ...
    if (updatedTicket.team) {
      try {
        // Get all team member IDs (including leader)
        const memberIds = updatedTicket.team.memberIds ? 
          (Array.isArray(updatedTicket.team.memberIds) ? updatedTicket.team.memberIds : [updatedTicket.team.memberIds]) : [];
        const allMemberIds = [...new Set([updatedTicket.team.leaderId, ...memberIds])];
        ...
        if (recipients.length > 0) {
          await sendAndStoreMultipleNotifications(recipients, {
            title: 'Ticket Completed ✅',
            body: `Ticket ${ticketId} has been reviewed and completed`,
            data: {
              ticketId: ticketId,
              type: 'ticket_completed',
            },
    ...
    if (adminUserId) {
      await logAdminAction(
        parseInt(adminUserId),
        'Completed ticket',
        `Marked ticket ${ticketId} as completed`,
        { ticketId }
      );
    }
